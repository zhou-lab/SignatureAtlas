...
```
 ...
 == Step A: MRMP layer =================================================
   ...
   [sa_02] + defs.tsv  (HAND-CURATED: in_group cells --> annotation name)
           ...
           >> CONSISTENCY GATE: re-derived P-numbers must equal defs.tsv <<
                                    |
                  102 contrasts --> 171 signed signatures
                                    |
 == Step B: projection + curation ======================================
   ...
   hm450.bed                     wg.bed
   141 signatures, 11,885 probes    166 signatures, 1,092 regions, 31,327 CpGs (0 dup)
```
...
reference_panel_samples.tsv                    # raw samples -> 87-cell panel pseudobulks
```
...
- **`wg.bed`** — every curated whole-genome CpG, 5 columns: `chr`, `beg0`, `end1`
  ...
  `region_name` (`<signature_id>.<gene>_<chr>_<firstCpG>`). **1,092** gene-named regions,
  **31,327** CpGs, **no duplicate coordinates**.
- **`hm450.bed`** — every selected HM450 probe, 5 columns: `chr`, `beg0`, `end1`
  (2 bp CpG), `probe_id` (cg number), `signature_id`. **11,885** probes across **141**
  signatures.
- **`reference_panel_samples.tsv`** — provenance for the 87-cell panel: every raw sample
  that fed each pseudobulk, 3 columns: `sample_name`, `pseudobulk_name`, `source` (Loyfer /
  BLUEPRINT / Tian / Zhou). 317 sample rows → 87 pseudobulks. Granularity by source:
  Loyfer/BLUEPRINT rows are individual WGBS samples; Tian rows are brain MajorTypes (each a
  single-cell pseudobulk) pooled into 14 groups; Zhou rows are Major pseudobulks taken as-is
  (`sample_name == pseudobulk_name`). Regenerated by `sa_06_panel_samples.sh`.
...
Currently **102 contrasts → 171 signatures** (each realized `+`/`-` direction is a signed
`signature_id`); **166** are realized as curated WGBS regions and **141** carry ≥5 HM450
...
Aux:     sa_06_panel_samples.sh   (-> reference_panel_samples.tsv provenance)
```

## TODO / Known issues

- **Curate the unannotated MRMPs in `mrmp_annotated.tsv`.** Only 171 of the 10,000 MRMPs
